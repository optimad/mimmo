...
int  inline posVectorFind( std::vector<T>	&vect,	T& target){
    int pos = -1;
    std::vector< T >::iterator it_find = std::find(vect.begin(),vect.end(),target);
    if (it_find!=vect.end())  {
...
int  inline posVectorFind( std::array<T, d> &vect, T& target){
    int pos = -1;
    std::array< T,d >::iterator it_find = std::find(vect.begin(),vect.end(),target);
    if (it_find!=vect.end()){
        pos = std::distance(vect.begin(),it_find);
    }
    return pos;
...
T findPosition(T & value, std::vector<T> & source){
    T result = -1;
	std::vector< T >::iterator itV = std::find(source.begin(), source.end(), value);
	if(itV != source.end())
