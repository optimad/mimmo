...
/*!
* \class PortOut
* \brief PortOut is the PIN base class dedicated to exchange data from a target class to another one (output).
* \ingroup core
* 
...
/*!
*  \class PortIn
*  \brief PortIn is the PIN base class dedicated to exchange data from another class to a target one (input).
*  \ingroup core
* 
*   PIN is an object member of BaseManipulation object.
*   hrough a PIN two base manipulation objects are linked together. One of these two
*   bjects is the parent object that gives an output to the other one that takes
*   this value as input. 
*/
class PortIn{
public:
